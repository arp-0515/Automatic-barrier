...
class NemuIpc():
    def _get_game_display_id(self) -> int:
        """
        Query the displayId where the game task is currently running.
        On MuMu with keep-app-alive enabled, the game runs on a virtual HDMI display
        (e.g. display 3/4/5), not the main display 0 which shows the launcher.

        Returns:
            int: displayId of the game task, or 0 if the game is not running
        """
        try:
            package = self.config.script.device.package_name.value
        except AttributeError:
            package = str(self.config.script.device.package_name)
        try:
            raw = self.adb_shell(
                "dumpsys activity activities | grep -E 'Display #[0-9]+ \\(activities|Task\\{'",
                timeout=10,
            )
        except Exception as e:
            logger.warning(f'Unable to query game displayId: {e}')
            return 0

        current = None
        for line in raw.splitlines():
            m = re.search(r'Display #(\d+) \(activities', line)
            if m:
                current = int(m.group(1))
                continue
            # A real task of a display is listed with 2-space indentation,
            # while references in the "top display focused" block are indented deeper
            if re.match(r'^  \* Task\{', line) and package in line and current is not None:
                return current
        return 0

    def _refresh_game_display(self) -> None:
        """
        Make sure nemu_ipc is capturing the display where the game is running.
        Re-query every 2 seconds and rebuild the ipc handle if the display changed.
        """
        now = time.time()
        if getattr(self, '_game_display_last_query', 0) and now - getattr(self, '_game_display_last_query', 0) < 2:
            return
        self._game_display_last_query = now
        display_id = self._get_game_display_id()
        if getattr(self, '_game_display_id', None) != display_id:
            if getattr(self, '_game_display_id', None) is not None:
                logger.info(f'Game display changed {getattr(self, "_game_display_id", None)} -> {display_id}')
            self._game_display_id = display_id
    ...
    def nemu_ipc(self) -> NemuIpcImpl:
        ...
        """
        display_id = getattr(self, '_game_display_id', None)
        if display_id is None:
            display_id = self._get_game_display_id()
            self._game_display_id = display_id
        logger.info(f'NemuIpc use display_id={display_id} for game capture')
    ...
    def screenshot_nemu_ipc(self):
        ...
        try:
            ...
            # The game display may not be ready yet, e.g. right after app_start
            # when the game is recreated on a new virtual HDMI display.
            # Force re-query the game display, rebuild the ipc handle and retry once.
            logger.warning(f'NemuIpc screenshot failed ({e}), re-querying game display and retrying')
            self._game_display_last_query = 0
            if has_cached_property(self, 'nemu_ipc'):
                self.nemu_ipc_release()
            self._refresh_game_display()
            self.sleep(2)
